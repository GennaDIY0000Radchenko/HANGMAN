    ...
    if start:
        ...
        def end_g():
            ...
            for h in range(0, len(secret_word)):
                if secret_word[h] in screen_word:
                    sc.blit(pg.font.Font(CHILLER, 100).render(secret_word[h], True, white), (s_x + d_x * h, s_y))
                else:
                    sc.blit(pg.font.Font(CHILLER, 100).render(secret_word[h], True, red), (s_x + d_x * h, s_y))
        ...
        while close:
            for m in pg.event.get():
                if m.type == pg.QUIT:
                    ...
                    pos = pg.mouse.get_pos()
                    if 1111 <= pos[0] <= 1455 and 435 <= pos[1] <= 575:
                        sys.exit()
                    elif 703 <= pos[0] <= 1048 and 435 <= pos[1] <= 575:
                        choice = False
                        first = True
