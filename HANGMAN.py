import pygame as pg
import sys
import random
import time
pg.init()
white = (255, 255, 255)
black = (0, 0, 0)
red = (235, 28, 36)
max_heart = 5
start_heart = 3
now_heart = 3
score = 0

sc = pg.display.set_mode((1540, 800))

full_exit = True
first = True
start_button = False
save = False
settings = False

CHILLER = 'data/CHILLER.ttf'
words = 'data/words.txt'
screen_start = pg.image.load('data/first_start.png')
play = pg.image.load('data/play.png')
next = pg.image.load('data/next.png')
next.set_colorkey(white)
exit = pg.image.load('data/exit.png')
exit.set_colorkey(white)
new_game = pg.image.load('data/new_game.png')
new_game.set_colorkey(white)
red_heart = pg.image.load('data/red_heart.png')
red_heart.set_colorkey(white)
grey_heart = pg.image.load('data/grey_heart.png')
grey_heart.set_colorkey(white)
metal = pg.image.load('data/metal.jpg')
left_arrow = pg.image.load('data/left_arrow.png')
left_arrow.set_colorkey(white)
right_arrow = pg.image.load('data/right_arrow.png')
right_arrow.set_colorkey(white)

while full_exit:
    sc.blit(screen_start, (0, 0))
    pg.display.update()
    while first:
        for i in pg.event.get():
            if i.type == pg.QUIT:
                sys.exit()
            elif i.type == pg.MOUSEBUTTONDOWN and i.button == 1:
                pos = pg.mouse.get_pos()
                if 110 <= pos[0] <= 534 and 120 <= pos[1] <= 274:
                    first = False
                    start_button = True
                elif 44 <= pos[0] <= 590 and 286 <= pos[1] <= 447:
                    settings = True
                    first = False
                elif 110 <= pos[0] <= 534 and pos[1] >= 450:
                    sys.exit()

        pg.time.wait(10)

    if start_button:
        def new_word():
            guessed_letters = []
            user_letters = []
            word = random.choice(open(words, 'r').readline().split()).upper()
            screen = ["_" for i in word]
            return guessed_letters, word, screen, user_letters

        def update_new_letter(screen):
            sc.blit(play, (0, 0))
            sc.blit(pg.font.Font(CHILLER, 90).render(secret_word, True, black), (0, 0))
            d_x = 65
            s_x = 1095 - len(secret_word) * d_x / 2
            s_y = 300
            for i in range(0, len(secret_word)):
                sc.blit(pg.font.Font(CHILLER, 100).render(screen[i], True, white), (s_x+d_x*i, s_y))

        def keyboard(user_letters):
            d_x = 89.5
            d_y = 125
            line = list('QWERTYUIOPASDFGHJKLZXCVBNM')
            for i in range(0, 26):
                s_x = 661
                s_y = 425
                if i <= 9:
                    if line[i] in user_letters:
                        sc.blit(pg.font.Font(CHILLER, 90).render(line[i], True, red), (s_x+d_x*i, s_y))
                    else:
                        sc.blit(pg.font.Font(CHILLER, 90).render(line[i], True, white), (s_x+d_x*i, s_y))
                elif 10 <= i <= 18:
                    s_x += d_x/2
                    s_y += d_y
                    if line[i] in user_letters:
                        sc.blit(pg.font.Font(CHILLER, 90).render(line[i], True, red), (s_x+d_x*(i-10), s_y))
                    else:
                        sc.blit(pg.font.Font(CHILLER, 90).render(line[i], True, white), (s_x+d_x*(i-10), s_y))
                else:
                    s_x += 1.5*d_x
                    s_y += d_y*2
                    if line[i] in user_letters:
                        sc.blit(pg.font.Font(CHILLER, 90).render(line[i], True, red), (s_x+d_x*(i-19), s_y))
                    else:
                        sc.blit(pg.font.Font(CHILLER, 90).render(line[i], True, white), (s_x + d_x * (i - 19), s_y))

        def hearts():
            d_x = 70
            s_x = 1520-start_heart*d_x
            s_y = 20
            for i in range(0, start_heart):
                if i < start_heart-now_heart:
                    sc.blit(grey_heart, (s_x+d_x*i, s_y))
                else:
                    sc.blit(red_heart, (s_x+d_x*i, s_y))

        def print_score():
            screen_score = "SCORE:" + str(round(score))
            sc.blit(pg.font.Font(CHILLER, 90).render(screen_score, True, white), (665, 5))

        def new_level(screen_word):
            update_new_letter(screen_word)
            keyboard(user_letters)
            hearts()
            print_score()

        def end_g():
            sc.blit(metal, (0, 0))
            screen_score = "SCORE:" + "  " + str(round(score))
            sc.blit(pg.font.Font(CHILLER, 90).render(screen_score, True, white), (30, 200))

            d_x = 50
            s_x = 300
            s_y = 300
            for i in range(0, len(secret_word)):
                if secret_word[i] in screen_word:
                    sc.blit(pg.font.Font(CHILLER, 100).render(secret_word[i], True, white), (s_x+d_x*i, s_y))
                else:
                    sc.blit(pg.font.Font(CHILLER, 100).render(secret_word[i], True, red), (s_x + d_x * i, s_y))

            sc.blit(pg.font.Font(CHILLER, 90).render('KILLER:', True, white), (30, 300))
            sc.blit(exit, (162, 490))
            sc.blit(new_game, (10, 640))

        if not save:
            save = False
            guessed_letters, secret_word, screen_word, user_letters = new_word()
            now_heart = 0 + start_heart
        new_level(screen_word)
        pg.display.update()

        while start_button:
            for i in pg.event.get():
                if now_heart == 0:
                    end_game = True
                else:
                    end_game = False

                    if len(guessed_letters) == len(secret_word):
                        end_level = True
                    else:
                        end_level = False

                if i.type == pg.QUIT:
                    sys.exit()
                elif i.type == pg.MOUSEBUTTONDOWN and i.button == 1:
                    pos = pg.mouse.get_pos()
                    if 0 <= pos[0] <= 72 and 0 <= pos[1] <= 56:
                        save = True
                        first = True
                        start_button = False
                elif i.type == pg.KEYDOWN:
                    if i.key == pg.K_ESCAPE:
                        first = True
                        start_button = False
                        save = True

                    elif 97 <= i.key <= 122 and not end_level and not end_game:
                        letter = chr(i.key).upper()
                        if letter not in user_letters:
                            user_letters.append(letter)

                            minus_heart = True
                            for a in range(0, len(secret_word)):

                                if secret_word[a] == letter and a not in guessed_letters:
                                    minus_heart = False
                                    guessed_letters.append(a)

                                    if len(guessed_letters) == len(secret_word):
                                        end_level = True
                                    else:
                                        end_level = False

                                    for a in range(0, len(guessed_letters)):
                                        screen_word.pop(guessed_letters[a])
                                        screen_word.insert(guessed_letters[a], secret_word[guessed_letters[a]])

                                    update_new_letter(screen_word)

                            if minus_heart:
                                now_heart -= 1
                                if now_heart == 0:
                                    end_game = True
                                else:
                                    end_game = False
                            else:
                                score += now_heart

                            hearts()
                            print_score()

                        keyboard(user_letters)

                if end_level:
                    sc.blit(next, (145, 450))
                    sc.blit(exit, (145, 600))
                    if i.type == pg.MOUSEBUTTONDOWN and i.button == 1:
                        pos = pg.mouse.get_pos()
                        if 145 <= pos[0] <= 497 and 600 <= pos[1] <= 742:
                            first = True
                            start_button = False
                        elif 145 <= pos[0] <= 497 and 450 <= pos[1] <= 592:
                            guessed_letters, secret_word, screen_word, user_letters = new_word()
                            new_level(screen_word)

                    elif i.type == pg.KEYDOWN:
                        if i.key == pg.K_ESCAPE:
                            first = True
                            start_button = False
                        elif i.key == pg.K_SPACE:
                            guessed_letters, secret_word, screen_word, user_letters = new_word()
                            new_level(screen_word)

                if end_game:
                    save = False
                    end_g()
                    if i.type == pg.MOUSEBUTTONDOWN and i.button == 1:
                        pos = pg.mouse.get_pos()
                        if 162 <= pos[0] <= 509 and 490 <= pos[1] <= 632:
                            score = 0
                            now_heart += start_heart
                            first = True
                            start_button = False
                        elif 10 <= pos[0] <= 672 and 640 <= pos[1] <= 782:
                            score = 0
                            now_heart += start_heart
                            guessed_letters, secret_word, screen_word, user_letters = new_word()
                            new_level(screen_word)

                    elif i.type == pg.KEYDOWN:
                        if i.key == pg.K_ESCAPE:
                            score = 0
                            now_heart += start_heart
                            first = True
                            start_button = False
                        elif i.key == pg.K_SPACE:
                            score = 0
                            now_heart += start_heart
                            guessed_letters, secret_word, screen_word, user_letters = new_word()
                            new_level(screen_word)

                pg.display.update()

            pg.time.wait(10)

    elif settings and ((not save) or (len(user_letters) == 0 and score == 0)):
        save = False


        def new_heart_settings():
            d_x = 70
            s_x = 910
            s_y = 350
            sc.blit(play, (0, 0))
            for i in range(0, max_heart):
                if i < max_heart - start_heart:
                    sc.blit(grey_heart, (s_x + d_x * i, s_y))
                else:
                    sc.blit(red_heart, (s_x + d_x * i, s_y))
            if start_heart != max_heart:
                sc.blit(left_arrow, (846, 347))
            if start_heart != 1:
                sc.blit(right_arrow, (1265, 347))

        sc.blit(play, (0, 0))
        new_heart_settings()
        pg.display.update()
        while settings:
            for i in pg.event.get():
                if i.type == pg.QUIT:
                    sys.exit()
                elif i.type == pg.MOUSEBUTTONDOWN and i.button == 1:
                    pos = pg.mouse.get_pos()
                    if 0 <= pos[0] <= 72 and 0 <= pos[1] <= 56:
                        settings = False
                        first = True
                    elif 910 <= pos[0] <= 1256 and 350 <= pos[1] <= 410:
                        start_heart = max_heart - (pos[0]-910)//70
                        sc.blit(pg.font.Font(CHILLER, 90).render(str(start_heart), True, white), (pos[0], pos[1]))
                    elif 846 <= pos[0] <= 901 and 347 <= pos[1] <= 411 and start_heart != max_heart:
                        start_heart += 1
                    elif 1256 <= pos[0] <= 1311 and 347 <= pos[1] <= 411 and start_heart != 1:
                        start_heart -= 1
                    new_heart_settings()

                pg.display.update()

            pg.time.wait(10)
    first = True
